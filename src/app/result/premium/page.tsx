  ...
  return (
    <div className="min-h-screen bg-[#E5E0D5] flex justify-center pb-12 pt-0 sm:pt-4 selection:bg-[#D4AF37] selection:text-white">
      ...
      <div className="w-full max-w-lg bg-white text-[#4A4036] font-serif shadow-2xl min-h-screen pb-24 relative overflow-hidden">
        ...
        {/* 7. Upsell Banner (Standard buyers only) */}
        {plan === 'standard' && (
          <section className="px-6 py-14 bg-gradient-to-b from-[#FAF4EA] to-[#F5EDE0] border-t border-[#E8DCC4]">
            <div className="text-center mb-6">
              <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#CFA770] flex items-center justify-center shadow-lg">
                <Crown className="w-6 h-6 text-white" />
              </div>
              <h3 className="text-lg font-bold tracking-widest text-[#2C241B] mb-2">プレミアム鑑定のご案内</h3>
              <p className="text-xs text-[#8A7A6A] tracking-wider leading-relaxed">
                {customerName}様の鑑定書をさらに深めませんか？
              </p>
            </div>

            <div className="bg-white rounded-lg border border-[#D4AF37]/30 p-6 shadow-sm space-y-4 mb-6">
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-[#D4AF37] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-[#2C241B] tracking-wider">相性診断（1件分）</p>
                  <p className="text-[11px] text-[#8A7A6A] tracking-wider">気になる人との相性をカバラで鑑定</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-[#D4AF37] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-[#2C241B] tracking-wider">ラッキーアクション365日分</p>
                  <p className="text-[11px] text-[#8A7A6A] tracking-wider">毎日の具体的な開運アドバイス</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-[#D4AF37] flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-[#2C241B] tracking-wider">1年後フォローアップ鑑定権</p>
                  <p className="text-[11px] text-[#8A7A6A] tracking-wider">1年後に再鑑定して成長を確認</p>
                </div>
              </div>
            </div>
            ...
            <div className="text-center">
              <div className="mb-3">
                <span className="text-xs text-[#8A7A6A] line-through tracking-wider">通常 ¥2,980</span>
                <span className="text-2xl font-bold text-[#D4AF37] ml-2 tracking-wider">¥2,000</span>
                <span className="text-xs text-[#8A7A6A] tracking-wider ml-1">（差額のみ）</span>
              </div>
              <button
                onClick={async () => {
                  setIsUpgrading(true);
                  try {
                    ...
                    // Retrieve name/dob from the session to create a new checkout
                    const res = await fetch('/api/checkout', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ name: customerName, dob: '1990-01-01', plan: 'premium' }),
                    });
                    const data = await res.json();
                ...
                className="w-full flex items-center justify-center gap-2 py-4 bg-gradient-to-r from-[#D4AF37] to-[#CFA770] text-white hover:from-[#CFA770] hover:to-[#D4AF37] disabled:opacity-50 transition-all text-sm tracking-widest rounded-lg shadow-md font-bold"
              >
                {isUpgrading ? (
                  <span className="animate-pulse">アップグレード準備中...</span>
                ) : (
                  <>プレミアムにアップグレード <ArrowRight className="w-4 h-4" /></>
                )}
              </button>
              <p className="text-[10px] text-[#8A7A6A] mt-3 tracking-wider">※スタンダード鑑定の全内容に加えて3つの特典が追加されます</p>
            </div>
          </section>
        )}
